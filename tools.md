...
```sh
docker compose -f /data/project/compose.yaml up -d
```

When `docker.service` systemd service is enabled, docker services will run if they were configured to at the time the above command was executed. Configuration is persistent in docker cache.
```
...
```sh
docker compose scale frontendService=3
```

Service configuration element `container_name` can not be used when scaling. Autogenerated names consist of `projectName, serviceName, index`.

```
docker compose logs project-frontendService-1
```
